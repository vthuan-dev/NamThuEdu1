...
export interface AssignExam {
  ...
  ageGroup?: string;
  /** Thời lượng làm bài (phút). Dùng để tính hạn chót tối thiểu: học viên phải
   *  có đủ thời gian làm hết đề trước khi hết hạn, nếu không thì giao đề mà
   *  không ai kịp làm. */
...
/** Khoảng dư sau thời lượng đề, để học viên còn kịp vào và nộp. */
const DEADLINE_BUFFER_MINUTES = 10;

/**
 * Đổi Date → chuỗi cho <input type="datetime-local">.
 *
 * KHÔNG dùng toISOString(): nó đổi sang UTC nên với giờ Việt Nam (UTC+7) sẽ ra
 * sớm hơn 7 tiếng, khiến ô lịch chặn sai và giáo viên không chọn được giờ hợp lệ.
 */
...
export function AssignModal({ open, exams, onClose, onAssigned }: AssignModalProps) {
  ...
  // ── GIỚI HẠN LỊCH ───────────────────────────────────────────────────────
  // Trước đây hai ô lịch không có min/max nên giáo viên chọn được cả ngày đã
  // qua, và hạn chót chỉ cần lớn hơn giờ mở 1 phút là hợp lệ — nghĩa là giao
  // được một đề 40 phút với hạn chót sau 5 phút, không ai kịp làm.
  //
  // Thời lượng lấy theo đề DÀI NHẤT: giao nhiều đề một lượt thì hạn chót phải
  // đủ cho đề nặng nhất, không phải đề đầu tiên.
  const examMinutes = useMemo(() => {
    const list = exams
  ...
  // Mốc "bây giờ" chốt lúc mở modal. Không dùng new Date() trực tiếp trong
  // render: mỗi lần render ra một giá trị khác nên min của ô lịch nhảy liên
  // tục, và giá trị giáo viên vừa chọn có thể bị chính nó làm cho không hợp lệ.
  const [now, setNow] = useState<Date>(() => new Date());
  useEffect(() => {
    if (open) setNow(new Date());
  }, [open]);

  const minStart = useMemo(() => toLocalInput(now), [now]);

  /**
   * Hạn chót tối thiểu = (giờ mở làm bài, hoặc bây giờ nếu để trống)
   *                      + thời lượng đề + khoảng dư.
   */
  const minDeadline = useMemo(() => {
    const base = startTime && !Number.isNaN(new Date(startTime).getTime())
      ? new Date(startTime)
      : now;
    return toLocalInput(addMinutes(base, examMinutes + DEADLINE_BUFFER_MINUTES));
  }, [startTime, now, examMinutes]);

  /** Hạn chót đang sớm hơn mức tối thiểu → chặn giao. */
  const deadlineTooEarly = Boolean(
    deadline &&
      !Number.isNaN(new Date(deadline).getTime()) &&
      new Date(deadline).getTime() < new Date(minDeadline).getTime(),
  );

  /** Giờ mở làm bài nằm trong quá khứ. */
  ...
  const scheduleInvalid = deadlineTooEarly || startInPast;
  const canSubmit =
  ...
  const missingRequirements = useMemo(() => {
    ...
    if (startInPast) {
      items.push("Thời điểm mở làm bài đang ở quá khứ.");
    }
    if (deadlineTooEarly) {
      // Nói rõ con số, vì giáo viên không có cách nào tự đoán ra mức tối thiểu.
      items.push(
        examMinutes > 0
          ? `Hạn chót quá sớm: đề dài ${examMinutes} phút, cần thêm ${DEADLINE_BUFFER_MINUTES} phút dự phòng.`
          : "Hạn chót phải sau thời điểm mở làm bài.",
      );
    }
    return items;
  }, [exams.length, hasTarget, mode, deadline, startInPast, deadlineTooEarly, examMinutes]);
                        ...
                        <div key={c.id} className="space-y-1">
                          <div
                            className={`group w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all duration-200 ${
                              sel ? "bg-white ring-1 ring-indigo-200 shadow-sm" : "hover:bg-white hover:shadow-sm"
                            }`}
                          >
                            {/* Phần click chọn lớp */}
                            <button
                            ...
                            >
                              <span
                                className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-all ${
                                  sel ? "bg-indigo-600 border-indigo-600" : "border-slate-300 bg-white group-hover:border-indigo-400"
                                }`}
                              >
                              ...
                              <span className="flex-1 min-w-0">
                                <span className={`block text-sm font-medium truncate ${sel ? "text-indigo-700" : "text-slate-800"}`}>
                                  {c.name}
                                </span>
                            ...
                            {/* Nút toggle dropdown xem thành viên */}
                            {c.studentCount > 0 && (
                              <button
                                type="button"
                                onClick={(e) => {
                          ...
                          {/* Danh sách học viên xổ xuống */}
                          {isExpanded && (
                            <div className="pl-9 pr-2 py-1 space-y-1 border-l-2 border-indigo-50 ml-7 animate-fadeIn">
                              {classStudents.length === 0 ? (
                                <p className="text-[11px] text-slate-400 py-1">Đang tải hoặc chưa có thông tin học viên...</p>
                              ) : (
                                classStudents.map((s) => (
                                  <div
                                    key={s.id}
                                  ...
                                  >
                                    <span className="w-6 h-6 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center text-[10px] font-medium shrink-0 overflow-hidden">
                                      {s.avatarUrl ? (
                                        <img
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <span className="block text-xs font-medium text-slate-700 truncate">{s.name}</span>
                                      {s.phone && <span className="block text-[10px] text-slate-400 truncate">{s.phone}</span>}
                                    </div>
                                  </div>
                                ))
                              )}
          ...
          <div className="px-5 py-4 space-y-4">
            ...
            <div className="rounded-lg bg-slate-50 ring-1 ring-slate-100 p-3.5 space-y-3">
              <div className="space-y-1.5">
                ...
                {/* min: trình duyệt tự làm mờ ngày/giờ đã qua trong lịch. */}
                <input
                  type="datetime-local"
                  value={startTime}
                  min={minStart}
                  onChange={(e) => setStartTime(e.target.value)}
                  className={`${inputCls} ${startInPast ? "border-rose-300 bg-rose-50/40 focus:border-rose-400 focus:ring-rose-200" : ""}`}
                />
                <p className={`text-[11px] ${startInPast ? "text-rose-600 font-medium" : "text-slate-400"}`}>
                  {startInPast
                    ? "Thời điểm này đã qua — chọn lại từ hiện tại trở đi."
                    : "Để trống = học viên làm được ngay."}
                </p>
              </div>
              <div className="space-y-1.5">
                <label className={`${labelCls} flex items-center gap-1.5`}>
                ...
                {/* min tính từ giờ mở + thời lượng đề + dự phòng, nên lịch không
                    cho chọn một hạn chót mà học viên không thể làm xong. */}
                <input
                  type="datetime-local"
                  value={deadline}
                  min={minDeadline}
                  ...
                  className={`${inputCls} ${
                    deadlineTooEarly
                      ...
                      : !deadline
                        ? "border-amber-300 bg-amber-50/40 focus:border-amber-400 focus:ring-amber-200"
                        : ""
                  }`}
                />
                <p
                ...
                >
                  {deadlineTooEarly
                    ? `Quá sớm — sớm nhất là ${minDeadline.replace("T", " ")}.`
                    : !deadline
                      ? "Bắt buộc: chọn hạn chót trước khi giao đề."
                      : "Sau thời điểm này, học viên không làm được đề nữa."}
                </p>
                {/* Nút điền nhanh: giáo viên không phải tự cộng nhẩm thời lượng đề. */}
                {examMinutes > 0 && (
                  <button
                    type="button"
                    onClick={() => setDeadline(minDeadline)}
                    className="text-[11px] font-medium text-indigo-600 hover:text-indigo-700 hover:underline"
                  >
                    Dùng mức tối thiểu ({examMinutes} phút làm bài + {DEADLINE_BUFFER_MINUTES} phút dự phòng)
                  </button>
                )}
              </div>
            </div>
