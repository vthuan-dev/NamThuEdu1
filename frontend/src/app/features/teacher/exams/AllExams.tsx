          ...
          return (
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="px-4 py-2 border-b border-gray-100 bg-gray-50/50">
                <h2 className="text-xs font-bold text-gray-700 uppercase tracking-wider">Lọc theo nhóm tuổi học viên</h2>
              </div>
                ...
                {groups.map(({ key, label, icon: Icon, count, hint }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative px-4 py-3 text-left transition-colors cursor-pointer ${
                        isActive ? "bg-orange-50/30" : "hover:bg-gray-50/60"
                      }`}
                    >
                      {isActive && (
                      ...
                      <div className="flex items-center gap-1.5 mb-1">
                        <Icon className={`w-4 h-4 ${isActive ? "text-orange-600" : "text-gray-400"}`} strokeWidth={2.5} />
                        <span className={`text-xs font-bold ${isActive ? "text-orange-600" : "text-gray-700"}`}>
                          {label}
                        </span>
                      </div>
                      <div className="flex items-baseline gap-1.5">
                        ...
                        <span className={`text-[10px] ${isActive ? "text-orange-600 font-bold" : "text-gray-400"}`}>đề · {hint}</span>
                      </div>
                    </button>
                  );
                })}
                ...
                >
                  ...
                  <div className="relative p-4 flex-1 flex flex-col">
                    ...
                    <div className={`flex items-center justify-between gap-2 mb-3 min-h-[24px] ${isRecentlyCreated(exam.eCreated_at, 2) ? "pr-10" : ""}`}>
                      {isOwner ? (
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          ...
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-100 flex-shrink-0">
                        ...
                        <span
                          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-100 max-w-[140px] flex-shrink-0"
                    ...
                    <div className="mb-2.5">
                      <span
                        className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-extrabold uppercase tracking-wide border-2 shadow-sm"
                        style={{
                          backgroundColor: `${typeInfo.color}15`,
                          color: typeInfo.color,
                          borderColor: `${typeInfo.color}35`,
                        }}
                      >
                        <span
                    ...
                    {/* Title */}
                    <h3 className="text-[15px] font-bold text-gray-900 leading-snug line-clamp-2 mb-1.5 transition-colors group-hover:text-indigo-700">
                      {exam.eTitle}
                    </h3>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[11px] mt-auto pt-2">
                      {(() => {
                        const qCount = countExamQuestions(exam);
                        if (qCount === 0) {
                          return (
                            <span className="inline-flex items-center gap-1 text-red-600 font-bold bg-red-50 px-2 py-0.5 rounded border border-red-200 animate-pulse">
                        ...
                        return (
                          <span className="inline-flex items-center gap-1 text-slate-800 font-bold bg-slate-100 px-2 py-0.5 rounded border border-slate-200">
                      ...
                      {exam.eDuration && (
                        <span className="inline-flex items-center gap-1 text-slate-800 font-semibold bg-slate-100 px-2 py-0.5 rounded border border-slate-200">
